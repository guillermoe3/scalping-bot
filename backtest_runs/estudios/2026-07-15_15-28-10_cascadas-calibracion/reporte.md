...
- evento: ret[i] = (c[i]-c[i-1])/c[i-1]; sigma = stdev de los 96 retornos 15m previos (retornos, no precios; sin lookahead; requiere i >= 97 y vol > 0); |ret[i]| > 3*sigma y flujo unidireccional: |2*taker_buy[i] - vol[i]| / vol[i] >= 0.6
...
- n_minimo_calibracion: regla dura: n_total < 50 -> conclusiones permitidas: solo 'descartar' o 'extender datos'
...
```json
{
  "BTCUSDT": {
    ...
    "bajista": {
      "1h": {
        "n": 1,
        "media": -0.0004886003160307563,
        "mediana": -0.0004886003160307563,
        "hit_rate": 0.0
      },
      "4h": {
        "n": 1,
        "media": -0.0014108519763808528,
        "mediana": -0.0014108519763808528,
        "hit_rate": 0.0
      },
      "8h": {
        "n": 1,
        "media": -0.0023494398175099664,
        "mediana": -0.0023494398175099664,
        "hit_rate": 0.0
      }
    },
    "n_total": 1,
    "nota": "n_total=1 < 50: conclusiones permitidas: solo 'descartar' o 'extender datos'"
  }
}
```
